...
type CompiledFunc = Box<dyn Fn(&State, &mut Store, &mut StackValue) -> Trap<Option<StackValue>>>;
...
impl Function {
  ...
  pub fn call(&self, state: &State, store: &mut Store, l0: &mut StackValue) -> Trap<Option<StackValue>> {
    match self.body {
